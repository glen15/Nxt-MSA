...
Step 3  SNS → SQS 구독 + 서버 환경변수 연결 → 토픽과 큐 연결 + 콜백 주소 활성화
Step 4  Lambda 생성 (receive-all)          → 재고 충전 + 발주 상태 업데이트
Step 5  전체 E2E 테스트                    → 주문 → 발주 → 생산 → 입고 → 재고 충전
```
...
## Step 3: SNS → SQS 구독 + 서버 환경변수 연결 (10분)
...
### 3-2. 서버에 receiving 토픽 ARN 연결

Cloud9에서 main-app의 `.env` 파일에 `RECEIVING_TOPIC_ARN`을 추가합니다:
...
`입고토픽`이 ✅로 표시되면 성공입니다. 이제 발주 메시지에 `callbackTopicArn`이 포함됩니다.
...
**구성** → **일반 구성** → **편집** → 타임아웃: **10초** → **저장**

> 💡 order-all Lambda는 공장 API 응답 대기(4초+)가 있어 **30초**가 필요했지만,
> receive-all Lambda는 **DynamoDB 업데이트만** 수행하므로 **10초**면 충분합니다.
>
> ⚠️ Lambda 타임아웃은 SQS **Visibility Timeout**(기본 30초)보다 작아야 합니다.
> Lambda가 아직 처리 중인데 Visibility Timeout이 먼저 만료되면
> 메시지가 큐에 다시 나타나서 **중복 처리**됩니다.
