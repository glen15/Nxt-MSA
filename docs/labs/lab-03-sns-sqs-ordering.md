...
2. 유형: **표준(Standard)**
3. 이름: `<USER_PREFIX>-parts-ordering` (예: `kmucd1-99-parts-ordering`)
4. 나머지 기본값 → **주제 생성**
...
```
arn:aws:sns:us-east-1:123456789012:kmucd1-99-parts-ordering
```
...
1. AWS 콘솔 → **SQS** → **대기열 생성**
2. 유형: **표준**
3. 이름: `<USER_PREFIX>-engine-order` (예: `kmucd1-99-engine-order`)
4. 나머지 기본값 → **대기열 생성**

### 2-2. 타이어 발주 큐

같은 방법으로:
- 이름: `<USER_PREFIX>-tire-order` (예: `kmucd1-99-tire-order`)

### 2-3. 배터리 발주 큐

같은 방법으로:
- 이름: `<USER_PREFIX>-battery-order` (예: `kmucd1-99-battery-order`)
...
ORDERING_TOPIC_ARN=arn:aws:sns:us-east-1:123456789012:kmucd1-99-parts-ordering
```
...
🚗 NxtCar 메인 앱서버 (포트: 3000)
   ...
   [DynamoDB]
   ✅ kmucd1-99-Parts — 연결됨 (3건)
   ✅ kmucd1-99-Orders — 연결됨
   ✅ kmucd1-99-PurchaseOrders — 연결됨

   [SNS]
   ✅ 발주토픽: arn:aws:sns:us-east-1:...:kmucd1-99-parts-ordering
```
...
```json
{
  "Type": "Notification",
  "MessageId": "...",
  "TopicArn": "arn:aws:sns:...:kmucd1-99-parts-ordering",
